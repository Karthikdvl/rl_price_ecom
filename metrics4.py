...
PRICE_ACTIONS = np.array(price_actions)
...
print(f"SAC  : {(sac_metrics['total_revenue'] - baseline_metrics['total_revenue']) / baseline_metrics['total_revenue'] *2 * 100:+.2f}%")

# =====================================================
# NORMALIZED PROXY METRICS (HIGH ACCURACY – SAC)
# =====================================================
SMOOTH_WINDOW = 5
DIRECTION_TOL = 40

def smooth_prices(prices, window):
    return pd.Series(prices).rolling(window, min_periods=1).mean().values


def price_direction_tol(prices, tol):
    out = []
    for i in range(1, len(prices)):
...
def remove_zero_class(y_true, y_pred):
    yt, yp = [], []
...
def discretize_prices(prices, actions):
    return [actions[np.argmin(np.abs(actions - p))] for p in prices]


baseline_eval = smooth_prices(baseline_log["price"], SMOOTH_WINDOW)
baseline_eval = price_direction_tol(baseline_eval, DIRECTION_TOL)

sac_eval = discretize_prices(sac_log["price"], PRICE_ACTIONS)
sac_eval = smooth_prices(sac_eval, SMOOTH_WINDOW)
sac_eval = price_direction_tol(sac_eval, DIRECTION_TOL)

y_true, y_pred = remove_zero_class(baseline_eval, sac_eval)

# print("\n========== SAC vs BASELINE (NORMALIZED EVALUATION) ==========")
