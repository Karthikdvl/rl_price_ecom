...
from metrics6 import evaluate_all_models
import io
...
@st.cache_resource
def load_rl_metrics(df):
    return evaluate_all_models(df)

rl_metrics = load_rl_metrics(base_df)
...
st.subheader("🤖 RL Model Evaluation Metrics")

selected_model = st.selectbox(
    "Select Model",
    list(rl_metrics.keys()),
    index=list(rl_metrics.keys()).index("SAC")
)

m = rl_metrics[selected_model]

c1, c2, c3, c4 = st.columns(4)

# c1.metric("Total Revenue", f"₹{m['total_revenue']:,.0f}")
c2.metric("Avg Reward", f"{m['avg_reward']:.2f}")
c3.metric("Price Volatility", f"{m['price_volatility']:.2f}")
c4.metric("Units Sold", f"{m['units_sold']:.0f}")

c5, c6, c7 = st.columns(3)

# c5.metric("Discounted Return", f"{m['discounted_return']:.2f}")
c6.metric("Price Change Freq", f"{m['price_change_freq']:.2%}")
c7.metric("Regret vs Best Price", f"₹{m['regret_vs_best_price']:,.0f}")
...
export_df["baseline_revenue"] = export_df["net_price"] * export_df["quantity"]
export_df["rl_revenue"] = export_df["rl_price"] * export_df["quantity"]

st.download_button(
    label="⬇️ Download Full Pricing Dataset (CSV)",
    data=to_csv_download(export_df),
    file_name="dynamic_pricing_full_data.csv",
    mime="text/csv"
)



st.subheader("📤 Export RL Model Metrics")

metrics_export_df = pd.DataFrame(rl_metrics).T.reset_index() \
    .rename(columns={"index": "Model"})

st.download_button(
    label="⬇️ Download RL Metrics (CSV)",
    data=to_csv_download(metrics_export_df),
    file_name="rl_model_metrics.csv",
    mime="text/csv"
)

st.download_button(
    label="⬇️ Download Model Comparison Table (CSV)",
    data=to_csv_download(comparison_df.reset_index()),
